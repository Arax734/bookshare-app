...
const DesiresScreen = () => {
  ...
  const { userId, userName } = route.params as DesiresScreenRouteParams;
  ...
  async function fetchDesiredBooks() {
    if (!userId) return;

    setLoading(true);
    try {
      const desiresQuery = query(collection(db, 'bookDesire'), where('userId', '==', userId));
      ...
      const booksWithDetails = await Promise.all(
        desires.map(async (desire) => {
          try {
            ...
            console.error(`Błąd podczas pobierania danych książki ${desire.bookId}:`, error);
            return desire;
          }
        })
      );
  ...
  const renderBookItem = ({ item }: { item: DesiredBook }) => {
          ...
          <View style={styles.bookDetails}>
            <View style={styles.bookDetail}>
              <View style={styles.detailHeader}>
                <UserIcon width={12} height={12} fill="#9333ea" />
            ...
            <View style={styles.bookMeta}>
              ...
              {book.language && (
                <View style={styles.metaItem}>
                  <Svg width={12} height={12} viewBox="0 0 20 20" fill="#9333ea">
                    <Path
                      fillRule="evenodd"
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={onRefresh}
              colors={['#9333ea']} // Purple color to match the theme
              tintColor="#9333ea"
            />
          }
        />
...
const styles = StyleSheet.create({
  ...
  detailsButton: {
    backgroundColor: '#9333ea', // purple-600 to match the screen's theme
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 4,
  },
