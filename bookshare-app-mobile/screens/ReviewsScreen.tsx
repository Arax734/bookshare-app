...
const ReviewsScreen = () => {
  ...
  const { userId, userName } = route.params as ReviewsScreenRouteParams;
  const insets = useSafeAreaInsets();
  const { user } = useAuth();

  const [reviews, setReviews] = useState<ReviewWithBookDetails[]>([]);
  const [filteredReviews, setFilteredReviews] = useState<ReviewWithBookDetails[]>([]);
  ...
  async function fetchReviews() {
    ...
    try {
      ...
      const reviewsWithBooks = await Promise.all(
        reviewsData.map(async (review) => {
          try {
            ...
            console.error(`Błąd podczas pobierania danych książki ${review.bookId}:`, error);
            return {
              ...review,
              bookTitle: 'Książka niedostępna',
              bookAuthor: 'Autor nieznany',
      ...
      setFilteredReviews(reviewsWithBooks);
    } catch (error) {
  ...
  const renderReviewItem = ({ item: review }: { item: ReviewWithBookDetails }) => {
      ...
      <View style={styles.reviewCard}>
        <View style={styles.reviewCardHeader}>
          <Text style={styles.bookTitle} numberOfLines={1}>
            {review.bookTitle && review.bookTitle.length > 40
              ? review.bookTitle.substring(0, 37) + '...'
          ...
          <View style={styles.reviewDetails}>
            <View style={styles.reviewDetail}>
              <View style={styles.detailHeader}>
                <UserIcon width={12} height={12} fill="#4f46e5" />
                <Text style={styles.detailLabel}>Autor:</Text>
              </View>
              <Text style={styles.detailText}>{review.bookAuthor || 'Autor nieznany'}</Text>
            </View>
            ...
            <View style={styles.bookMeta}>
              {review.bookYear && (
                <View style={styles.metaItem}>
                  ...
                  <Text style={styles.metaText}>{review.bookYear}</Text>
                </View>
              )}
              {review.language && (
                <View style={styles.metaItem}>
                  <Svg width={12} height={12} viewBox="0 0 20 20" fill="#4f46e5">
                    <Path
                      fillRule="evenodd"
                  ...
                  <Text style={[styles.metaText, styles.capitalize]}>{review.language}</Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.reviewContentContainer}>
              {review.comment ? (
                ...
                <Text style={styles.noReviewContent}>Brak treści recenzji</Text>
              )}
            </View>
