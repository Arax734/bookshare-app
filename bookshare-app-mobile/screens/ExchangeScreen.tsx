...
import { TabParamList, TabRouteProps } from '../types/navigation';
...
type ExchangeScreenRouteProp = TabRouteProps<'Exchange'>;
...
const ExchangeScreen = () => {
      ...
      const booksWithDetails = await Promise.all(
        books.map(async (book) => {
          try {
            ...
            console.error(`Błąd podczas pobierania danych książki ${book.bookId}:`, error);
            return book;
          }
        })
      );
  ...
  useEffect(() => {
    setLoading(true);
    fetchExchangeBooks().finally(() => setLoading(false));
  }, [userId]);

  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    fetchExchangeBooks().finally(() => setRefreshing(false));
  }, [userId]);
  ...
  const renderBookItem = ({ item }: { item: ExchangeBook }) => {
          ...
          <View style={styles.bookDetails}>
            <View style={styles.bookDetail}>
              <View style={styles.detailHeader}>
                <UserIcon width={12} height={12} fill="#16a34a" />
